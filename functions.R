########## Creation d'une fonction qui prend les r2 pour CCM
fun_ccm_plot_r2 <- function(correlation_df, var, time_frame,  metric_name, indicator){
  ...
  if(metric_name == "glmm"){
    if(indicator=='parity'){
      ...
      correlation_df$r2_modif<-as.numeric(ifelse(correlation_df$correlation<0,correlation_df$r2*(-1), correlation_df$r2))
    }
  }
  
  if(length(unique(correlation_df$abs_corr))!=1){ # to deal with case all correlation values are NAs
    
    most_corr <- correlation_df %>% filter(r2 == max(r2, na.rm = T))
    most_corr2 <- correlation_df %>% arrange(desc(r2)) %>% filter(r2 >= most_corr$r2 * 0.9)
  } else {
    most_corr <- most_corr2 <- correlation_df[1,]
  }
  
  ccm_plot <- ggplot(data = correlation_df, aes(as.numeric(time_lag_1), as.numeric(time_lag_2), fill = as.numeric(r2_modif))) +
  ...
  if(metric_name=="Spearman"){
    ccm_plot <- ccm_plot + 
      scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, limit = c(-.1,.1), space = "Lab", name = "Spearman correlation", na.value = "grey") #+ ##midpoint = 0 --> effet limite variable
  } else if(metric_name=="glmm"){
    ccm_plot <- ccm_plot +  
      common_scale+ theme(legend.position = "bottom")
  }
  return(ccm_plot)
}


########## Creation d'une fonction qui prend les r2 pour autres graphes

fun_plot_tile_univ_spatial_r2 <- function(correlation_df, metric_name , indicator, lc_source, type, xlabel){
  
  correlation_df <- correlation_df %>%
    mutate(abs_corr = ifelse(indicator=="abundance",correlation,correlation-1))%>%
    mutate(r2_modif=dplyr::case_when(indicator%in%c('parity')& abs(correlation)>=1~r2, 
                                     indicator%in%c('parity')& abs(correlation)<1~r2*(-1), 
                                     indicator=="abundance"&correlation>=0~r2, indicator=="abundance"&correlation<0~r2*(-1)))
  
  correlation_df <- correlation_df %>%
    mutate(indicator = forcats::fct_relevel(indicator, c("parity","abundance")))
  ...
  if(metric_name == "glmm"){
    ...
    # } else if (indicator == "abundance"){
    p <- p + scale_fill_gradient2(low = "blue", high = "red", mid = "white", limit = c(-0.1,0.1),midpoint = 0, space = "Lab", name = "r2 ajuste", na.value="grey")
    #p <- p + scale_fill_gradient2(low = "blue", high = "red", mid = "white", limit = c(-0.6, 0.6),midpoint = 0, space = "Lab", name = "r2 ajuste", na.value="grey")
    #}
